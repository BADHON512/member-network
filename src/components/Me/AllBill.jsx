...
export default function AllBill() {
  ...
  useEffect(() => {
    async function name() {
      const { data } = await axios.get(
        "https://member-network-server.vercel.app/api/v2/get-dis-users"
      );
      setUsers(data.users);
    }
    name();
  ...
  const columns = [
    ...
    { field: "type", headerName: "Type", minWidth: 70, flex: 0.7 },
  ...
  const filteredUsers = Users?.filter((user) => user.month === selectedMonth && user.type==="Dish-bill");

  const Total = filteredUsers?.reduce(
    (accumulator, currentUser) => accumulator + parseFloat(currentUser.price),
    0
  );
  ...
  return (
    <>
      <div className="h-[8vh] w-full p-3 flex gap-x-20 items-center">
        <select
          className="text-[17px] outline-none font-semibold"
          value={selectedMonth}
